      ...
      <ClientLayout>
        <div className="mx-auto w-full max-w-7xl space-y-6">
          ...
          <section className="grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_minmax(340px,0.45fr)]">
            <div className="relative overflow-hidden rounded-[1.75rem] bg-[#836A5D] p-6 text-[#FFF8F0] shadow-[0_30px_90px_-58px_rgba(47,40,35,0.95)] sm:p-8">
              ...
              <p className="relative text-[0.68rem] font-bold uppercase tracking-[0.24em] text-[#C8B79E]">Bienvenida</p>
              <div className="relative mt-5 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  <h1 className="font-alilato text-4xl font-bold leading-[0.95] tracking-[0] text-[#FFF8F0] sm:text-5xl">
                    Hola, {firstName || "Cristopher"}
                  </h1>
                  <p className="mt-4 max-w-[58ch] text-sm leading-6 text-[#faf3ea]/68">
                ...
                <div className="grid grid-cols-3 gap-2 sm:min-w-[24rem]">
                  {[
                    ...
                    <Link
                      key={to}
                      to={to}
                      className="group flex min-h-[6.8rem] flex-col justify-between rounded-2xl border border-[#faf3ea]/12 bg-[#faf3ea]/8 p-3 no-underline transition-all hover:-translate-y-0.5 hover:bg-[#faf3ea]/12"
                    >
                      <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#faf3ea]/12 text-[#FFF8F0]">
                        <Icon size={18} />
                      </span>
                      <span className="flex items-center justify-between text-xs font-semibold text-[#faf3ea]/78">
                        {label}
            ...
            <aside className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
              {[
                ...
                <div key={stat.label} className="rounded-[1.35rem] border border-[#836A5D]/12 bg-white/58 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.74),0_18px_50px_-38px_rgba(84,67,49,0.58)] backdrop-blur">
                  <p className="text-[0.66rem] font-bold uppercase tracking-[0.18em] text-[#836A5D]/48">{stat.label}</p>
                  ...
                  {stat.loading ? (
                    ...
                    <p className="mt-3 truncate text-xl font-bold text-[#4A3329]">{stat.value}</p>
                  )}
                </div>
              ))}
            </aside>
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="mb-3 flex items-end justify-between gap-3">
                  <p className="text-[0.68rem] font-bold uppercase tracking-[0.18em] text-[#836A5D]/52">Mi membresía</p>
                  <Link to="/app/checkout" className="text-xs font-semibold text-[#836A5D] no-underline hover:text-[#4A3329]">
                    Ver planes
                  </Link>
                </div>
                {loadingMembership ? (
                  ...
                  <div className="rounded-[1.35rem] border border-[#836A5D]/14 bg-white/54 p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.74),0_22px_60px_-42px_rgba(84,67,49,0.58)]">
                    <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                      <div>
                        <p className="text-lg font-bold text-[#4A3329]">No tienes membresía activa</p>
                        <p className="mt-1 max-w-[48ch] text-sm text-[#715B50]/62">
                          Elige un plan para reservar clases y ver tus créditos disponibles.
                        </p>
                      </div>
                      <Button asChild className="bg-[#836A5D] text-[#FAF3EA] hover:bg-[#6C5147]">
              ...
              <div>
                <div className="mb-3 flex items-end justify-between gap-3">
                  <p className="text-[0.68rem] font-bold uppercase tracking-[0.18em] text-[#836A5D]/52">Próximas clases</p>
                  <Link to="/app/classes" className="text-xs font-semibold text-[#836A5D] no-underline hover:text-[#4A3329]">
                    Reservar
                  </Link>
                </div>
                {loadingBookings ? (
                  ...
                  <div className="rounded-[1.35rem] border border-dashed border-[#836A5D]/18 bg-[#836A5D]/[0.035] p-8 text-center">
                    <p className="text-base font-semibold text-[#4A3329]">No tienes clases próximas</p>
                    <p className="mt-1 text-sm text-[#715B50]/58">Reserva desde el calendario semanal del estudio.</p>
                    <Button asChild className="mt-5 bg-[#836A5D] text-[#FAF3EA] hover:bg-[#6C5147]">
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {upcomingBookings.map((b) => (
                      <div key={b.id} className="rounded-[1.15rem] border border-[#836A5D]/12 bg-white/58 p-4 shadow-[0_18px_48px_-38px_rgba(84,67,49,0.5)]">
                        <div className="flex items-center gap-4">
                          <div className="shrink-0 rounded-2xl bg-[#836A5D] px-3 py-2 text-center text-[#FFF8F0]">
                            <p className="tabular text-lg font-bold leading-none">
                              {b.start_time ? format(safeParse(b.start_time), "HH:mm") : "—"}
                            </p>
                            <p className="mt-1 text-[10px] capitalize text-[#faf3ea]/65">
                              {b.start_time ? format(safeParse(b.start_time), "EEE d", { locale: es }) : ""}
                            </p>
                          </div>
                          <div className="min-w-0 flex-1">
                            <p className="truncate text-sm font-bold text-[#4A3329]">{b.class_type_name}</p>
                            <p className="mt-0.5 truncate text-xs text-[#715B50]/58">{b.instructor_name ?? b.class_type_name}</p>
                          </div>
                          <Badge
                            className={b.status === "waitlist"
                              ? "border border-[#C8B79E]/40 bg-[#C8B79E]/20 text-[#836A5D]"
                              : "bg-[#836A5D] text-[#FAF3EA]"}
                          >
            ...
            <aside className="space-y-6">
              ...
              {(noMembership || isLowCredits) && (
                <Link to="/app/checkout" className="block no-underline">
                  <div className="relative overflow-hidden rounded-[1.35rem] border border-[#836A5D]/14 bg-white/58 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.72),0_22px_60px_-42px_rgba(84,67,49,0.64)] transition-all hover:-translate-y-0.5">
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#836A5D]/10 text-[#836A5D]">
                          <Sparkles size={18} />
                        </div>
                        <div>
                          <p className="font-bold text-[#4A3329]">
                            {noMembership ? "Adquiere tu membresía" : "Renueva tu plan"}
                          </p>
                          <p className="mt-0.5 text-xs text-[#715B50]/58">
                            {noMembership
                      ...
                      <ArrowRight size={16} className="shrink-0 text-[#836A5D]" />
                    </div>
                  </div>
                </Link>
              )}
              ...
              {pendingOrders.length > 0 && (
                <div>
                  <p className="mb-3 flex items-center gap-1.5 text-[0.68rem] font-bold uppercase tracking-[0.18em] text-[#836A5D]/52">
                    <CreditCard size={12} />
                    {pendingOrders.length === 1 ? "Orden pendiente" : "Órdenes pendientes"}
                  </p>
                  <div className="space-y-3">
                    {pendingOrders.map((o: any) => (
                      <div key={o.id} className="rounded-[1.15rem] border border-[#836A5D]/12 bg-white/60 p-4 shadow-[0_18px_48px_-38px_rgba(84,67,49,0.5)]">
                        <div className="flex items-start justify-between gap-3">
                          <div className="space-y-0.5">
                            <p className="font-bold text-sm text-[#4A3329]">{o.plan_name}</p>
                            <p className="text-xs text-[#715B50]/58">
                            ...
                            {o.order_number && (
                              <p className="font-mono text-[10px] text-[#836A5D]/50">Orden: {o.order_number}</p>
                            )}
                          </div>
                          <Badge
                            variant="outline"
                        ...
                        {o.status === "pending_payment" && (
                          <Button asChild size="sm" className="mt-3 w-full bg-[#836A5D] text-[#FAF3EA] hover:bg-[#6C5147]">
                  ...
                  <div className="space-y-3">
                    {consultations.map((c) => (
                      <div key={c.id} className="rounded-[1.15rem] border border-amber-300/35 bg-amber-50/55 p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="space-y-0.5">
                            <p className="font-bold text-sm text-[#4A3329]">{c.complement_name}</p>
                            <p className="text-xs text-[#715B50]/58">Especialista: {c.specialist}</p>
                        ...
                        {c.notes && (
                          <p className="mt-2 text-xs italic text-[#715B50]/45">Nota: {c.notes}</p>
                        )}
                      </div>
                    ))}
                  </div>
