...
const CAT_COLORS: Record<ClassCat, { bg: string; text: string; border: string; dot: string }> = {
  pilates:   { bg: "bg-[#C8B79E]/15", text: "text-[#4a5638]",  border: "border-[#C8B79E]/40", dot: "bg-[#6b7a52]"  },
  bienestar: { bg: "bg-[#836A5D]/15", text: "text-[#5a4f46]",  border: "border-[#836A5D]/40", dot: "bg-[#7a6d62]"  },
  all:       { bg: "bg-[#836A5D]/[0.06]",      text: "text-[#2d2d2d]/70",   border: "border-[#836A5D]/20",     dot: "bg-[#836A5D]/40"   },
};
...
const MembershipBanner = ({ membership }: { membership: any }) => {
  ...
  return (
    <div className="rounded-[1.5rem] border border-[#836A5D]/14 bg-white/58 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.72),0_18px_50px_-40px_rgba(74,51,41,0.42)] backdrop-blur">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-bold text-[#4A3329]">{membership.planName ?? membership.plan_name}</p>
        ...
        <div className="grid grid-cols-2 gap-3 sm:min-w-[16rem]">
          <div className="rounded-2xl border border-[#836A5D]/10 bg-[#FAF3EA]/70 px-4 py-3">
            <div className={cn("text-2xl font-bold leading-none", colors.text)}>
              {isUnlimited ? "∞" : remaining}
            </div>
            <div className="mt-1 text-[0.66rem] font-semibold uppercase tracking-[0.16em] text-[#715B50]/60">
              {isUnlimited ? "Ilimitado" : "Clases"}
            </div>
          </div>
          {endDate && (
            <div className="rounded-2xl border border-[#836A5D]/10 bg-[#FAF3EA]/70 px-4 py-3 text-right">
              <div className="text-sm font-bold text-[#4A3329]">
    ...
    <ClientAuthGuard requiredRoles={["client"]}>
      <ClientLayout>
        <div className="mx-auto w-full max-w-7xl space-y-6">
          {/* Header */}
          <section className="relative overflow-hidden rounded-[2rem] border border-[#836A5D]/12 bg-[#FAF3EA]/70 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.74),0_24px_70px_-52px_rgba(74,51,41,0.6)] backdrop-blur sm:p-6">
            <div className="pointer-events-none absolute -right-20 -top-24 h-64 w-64 rounded-full bg-[#C8B79E]/20 blur-3xl" />
            <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <p className="text-[0.68rem] font-bold uppercase tracking-[0.24em] text-[#836A5D]/58">
                  Reserva semanal
                </p>
                <h1 className="mt-2 font-alilato text-3xl font-bold leading-none tracking-[0] text-[#4A3329] sm:text-4xl">
                  Elige tu clase
                </h1>
                <p className="mt-3 max-w-[56ch] text-sm leading-6 text-[#715B50]/72">
                  Revisa cupos por día y confirma el horario que mejor acompañe tu semana.
                </p>
              </div>
              <div className="flex items-center gap-2 self-start rounded-2xl border border-[#836A5D]/12 bg-white/62 p-1.5 shadow-[0_16px_42px_-34px_rgba(74,51,41,0.55)] lg:self-auto">
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => setWeekStart((w) => subWeeks(w, 1))}
                  className="h-11 w-11 rounded-xl text-[#4A3329] hover:bg-[#836A5D]/10"
                  aria-label="Semana anterior"
                >
                  <ChevronLeft size={18} />
                </Button>
                <div className="min-w-[12.5rem] px-3 text-center">
                  <p className="text-[0.65rem] font-bold uppercase tracking-[0.18em] text-[#836A5D]/55">Semana</p>
                  <p className="mt-0.5 text-sm font-bold text-[#4A3329]">
          ...
          <section className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.35fr)_minmax(280px,0.35fr)]">
            ...
            <div className="rounded-[1.5rem] border border-[#836A5D]/12 bg-white/55 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)]">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-[0.66rem] font-bold uppercase tracking-[0.16em] text-[#836A5D]/55">Disponibles</p>
                  {/* Gate: evita el flash de "0" antes de que carguen clases/membresía. */}
                  {(loadingClasses || loadingMembership) ? (
                    <Skeleton className="mt-2 h-8 w-12" />
                  ) : (
                    <p className="mt-2 text-3xl font-bold leading-none text-[#4A3329]">{bookableThisWeek}</p>
                  )}
                </div>
                <CalendarDays size={22} className="text-[#836A5D]/55" />
              </div>
            </div>
          ...
          {membershipCat && membershipCat !== "all" && (
            <div className="flex items-center gap-2 rounded-2xl border border-[#836A5D]/10 bg-white/42 px-4 py-3 text-xs">
              <CheckCircle2 size={13} className={CAT_COLORS[membershipCat].text} />
              <span className="text-[#715B50]/72">
          ...
          {/* Week grid */}
          <section className="overflow-hidden rounded-[2rem] border border-[#836A5D]/12 bg-white/48 shadow-[inset_0_1px_0_rgba(255,255,255,0.76),0_26px_78px_-56px_rgba(74,51,41,0.58)] backdrop-blur">
            <div className="overflow-x-auto">
              <div className="grid min-w-[980px] grid-cols-7 divide-x divide-[#836A5D]/10">
              {days.map((day, i) => (
                <div key={i} className="min-h-[36rem] bg-[#FAF3EA]/20">
                  <div className="sticky top-0 z-[1] border-b border-[#836A5D]/10 bg-[#FFF8F0]/92 px-4 py-4 text-center backdrop-blur">
                    <div className="text-[0.68rem] font-bold uppercase tracking-[0.18em] text-[#715B50]/58">{DAYS[i]}</div>
                    <div className="mt-1 text-2xl font-bold leading-none text-[#4A3329]">{format(day, "d")}</div>
                    <div className="mt-2 text-[0.68rem] font-semibold text-[#836A5D]/58">
                  ...
                  {loadingClasses ? (
                    ...
                    <div className="space-y-3 p-3">
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              ...
                              !isBooked && !disabled && cn(c.border, "cursor-pointer bg-white/72 shadow-[0_16px_42px_-34px_rgba(74,51,41,0.48)] hover:-translate-y-0.5 hover:border-[#836A5D]/35 hover:bg-[#FFF8F0]"),
                              !isBooked && isPast  && "cursor-not-allowed border-[#836A5D]/10 bg-white/20 opacity-42",
                              !isBooked && locked  && "cursor-not-allowed border-[#836A5D]/10 bg-white/24 opacity-45",
                            )}
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <div className="flex items-center gap-2">
                                  ...
                                  <p className={cn("truncate text-sm font-bold", isBooked ? "text-emerald-900" : "text-[#4A3329]")}>
                                    {cls.class_type_name}
                                  </p>
                                </div>
                                <p className="mt-3 flex items-center gap-1.5 text-lg font-bold leading-none text-[#4A3329]">
                                  <Clock size={14} className="text-[#836A5D]/58" />
                              ...
                              {locked && (
                                <span className="rounded-full bg-[#836A5D]/8 p-1 text-[#715B50]/45">
                                  <Lock size={13} />
                                </span>
                              )}
                            </div>
                            <div className="mt-4 flex items-center justify-between border-t border-[#836A5D]/10 pt-3">
                              <span className="text-[0.66rem] font-semibold uppercase tracking-[0.14em] text-[#715B50]/52">
                                50 min
                              </span>
                              <span className={cn(
                                ...
                                isBooked
                                  ...
                                  : disabled
                                    ? "bg-[#836A5D]/8 text-[#715B50]/42"
                                    : "bg-[#836A5D]/10 text-[#836A5D]"
                              )}>
                      ...
                      {classesForDay(day).length === 0 && (
                        <div className="rounded-2xl border border-dashed border-[#836A5D]/14 p-5 text-center">
                          <p className="text-xs font-semibold text-[#715B50]/52">Sin clases</p>
                        </div>
                      )}
                    </div>
                  )}
          ...
          {/* Legend — solo estados, no categorías (Pilates Room ofrece un único método) */}
          <div className="flex flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2 rounded-full border border-[#836A5D]/15 bg-white/55 px-3 py-1.5 text-xs font-semibold text-[#836A5D]">
              <CalendarDays size={12} /> Disponible
            </div>
            <div className="flex items-center gap-2 rounded-full border border-[#836A5D]/10 bg-[#836A5D]/[0.04] px-3 py-1.5 text-xs font-semibold text-[#715B50]/55">
