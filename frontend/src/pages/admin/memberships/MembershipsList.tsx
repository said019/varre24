            ...
            {isLoading
              ...
              : memberships.map((m) => {
                const catColors: Record<string, string> = {
                  pilates: "bg-[#E7C9CF]/15 text-[#E7C9CF] border-[#E7C9CF]/30",
                  bienestar: "bg-[#7C0116]/15 text-[#7C0116] border-[#7C0116]/30",
                  all: "bg-[#FFE4E8]/15 text-[#FFE4E8] border-[#FFE4E8]/30",
                };
                  ...
                  <TableRow key={m.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        {cat && cat !== "all" && (
                          <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full border capitalize ${catColors[cat] ?? "text-[#2B0911]/40 border-[#7C0116]/15"}`}>
                            {cat}
                          </span>
                        )}
                      </div>
          ...
          <DialogContent className="max-w-md">
            ...
            <form onSubmit={form.handleSubmit((d) => createMutation.mutate(d))} className="space-y-4">
              ...
              {(() => {
                ...
                return (
                  <div className="space-y-1 rounded-lg border border-[#E7C9CF]/15 bg-[#E7C9CF]/[0.03] p-3">
                ...
                return (
                  <div className="rounded-xl border border-[#7C0116]/20 bg-[#FFF1F3]/60 p-3 space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-[#2B0911]/60">{selPlan?.name}{hasCombo ? " + Complemento" : ""}</span>
                      {discountTotal ? (
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-[#7C0116] line-through">${total.toLocaleString("es-MX")}</span>
                          <span className="font-bold text-[#2B0911]">${discountTotal.toLocaleString("es-MX")}</span>
                        </div>
                      ) : (
                        <span className="font-bold text-[#2B0911]">${total.toLocaleString("es-MX")}</span>
                      )}
                    </div>
                    {isDiscount && (
