...
const Dashboard = () => {
  ...
  const metric = (label: string, value: number | undefined, icon: React.ReactNode, prefix = "", accent = "#C8B79E") => (
    <Card className="group overflow-hidden border-t-2 transition-all hover:-translate-y-0.5" style={{ borderTopColor: accent }}>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="font-alilato text-sm font-semibold text-[#6C5147]/70">{label}</CardTitle>
        <span className="rounded-xl bg-[#836A5D]/[0.07] p-2 transition-transform group-hover:scale-105" style={{ color: accent }}>
          {icon}
        </span>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <p className="tabular text-3xl font-bold tracking-[-0.01em] text-[#2f2823]">
            {prefix}{value ?? 0}
          </p>
        )}
      </CardContent>
      ...
      <AdminLayout>
        <div className="admin-page max-w-6xl">
          <section className="mb-6 grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
            <div className="rounded-[1.35rem] border border-[#836A5D]/15 bg-[#2f2823] p-5 text-[#f8f3ea] shadow-[0_28px_70px_-45px_rgba(47,40,35,0.9)] sm:p-6">
              <p className="mb-3 text-[11px] font-semibold uppercase tracking-[0.22em] text-[#bea98f]">
                Panel administrativo
              </p>
              <h1 className="font-alilato text-3xl font-bold leading-none tracking-[0] text-[#f8f3ea] sm:text-4xl">
                Dashboard
              </h1>
              <p className="mt-3 max-w-[58ch] text-sm leading-6 text-[#f5ecdb]/70">
            ...
            <button
              ...
              className="group flex min-h-[9.5rem] flex-col justify-between rounded-[1.35rem] border border-[#836A5D]/15 bg-white/65 p-5 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.72),0_20px_54px_-38px_rgba(84,67,49,0.48)] backdrop-blur sm:p-6"
            >
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#836A5D]/10 text-[#836A5D]">
                <CalendarDays size={18} />
              </span>
              <span>
                <span className="block text-sm font-semibold text-[#2f2823]">Abrir calendario</span>
                <span className="mt-1 flex items-center gap-1 text-xs text-[#836A5D]/70">
          ...
          <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {metric("Clases de hoy", stats?.classesToday, <CalendarDays size={18} />, "", "#C8B79E")}
            {metric("Membresías activas", stats?.activeMembers, <Users size={18} />, "", "#836A5D")}
            {metric("Ingresos del mes", stats?.monthlyRevenue, <DollarSign size={18} />, "$", "#8F7559")}
