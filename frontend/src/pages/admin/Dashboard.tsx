...
const Dashboard = () => {
  ...
  const metric = (label: string, value: number | undefined, icon: React.ReactNode, prefix = "", accent = "#C9A5A8") => (
    <Card className="group overflow-hidden border-t-2 transition-all hover:-translate-y-0.5" style={{ borderTopColor: accent }}>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="font-alilato text-sm font-semibold text-[#260910]/70">{label}</CardTitle>
        <span className="rounded-xl bg-[#3B0E1A]/[0.07] p-2 transition-transform group-hover:scale-105" style={{ color: accent }}>
          {icon}
        </span>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <p className="tabular text-3xl font-bold tracking-[-0.01em] text-[#1A060B]">
            {prefix}{value ?? 0}
          </p>
        )}
      </CardContent>
        ...
        <div className="admin-page max-w-6xl">
          <section className="mb-6 grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
            <div className="rounded-[1.35rem] border border-[#3B0E1A]/15 bg-[#1A060B] p-5 text-[#F3EFE9] shadow-[0_28px_70px_-45px_rgba(47,40,35,0.9)] sm:p-6">
              ...
              <h1 className="font-alilato text-3xl font-bold leading-none tracking-[0] text-[#F3EFE9] sm:text-4xl">
                Dashboard
              </h1>
              <p className="mt-3 max-w-[58ch] text-sm leading-6 text-[#EADCDD]/70">
            ...
            >
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#3B0E1A]/10 text-[#3B0E1A]">
                <CalendarDays size={18} />
              </span>
              <span>
                <span className="block text-sm font-semibold text-[#1A060B]">Abrir calendario</span>
                <span className="mt-1 flex items-center gap-1 text-xs text-[#3B0E1A]/70">
          ...
          <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {metric("Clases de hoy", stats?.classesToday, <CalendarDays size={18} />, "", "#C9A5A8")}
            {metric("Membresías activas", stats?.activeMembers, <Users size={18} />, "", "#3B0E1A")}
            {metric("Ingresos del mes", stats?.monthlyRevenue, <DollarSign size={18} />, "$", "#9C8A8B")}
