    ...
    <Card>
      ...
      <CardContent>
        {loadingSummary ? (
          ...
          <p className="text-2xl font-bold text-[#544331] tabular-nums">{value ?? "—"}</p>
        )}
      </CardContent>
    </Card>
  );
    ...
    <AuthGuard>
      <AdminLayout>
        <div className="admin-page max-w-6xl">
          <div className="mb-6">
            <h1 className="admin-title font-bold text-[#544331]">Referidos</h1>
            <p className="mt-1 text-sm text-[#5F4B3D]/55">
          ...
          {(loadingPending || pending.length > 0) && (
            <Card className="mb-6 border-amber-300/60 bg-amber-50/40">
              <CardHeader>
                <CardTitle className="text-base flex items-center gap-2 text-[#544331]">
                    ...
                    {pending.map((p) => (
                      <div key={p.id} className="flex flex-col gap-3 rounded-xl border border-amber-200 bg-white/70 p-3.5 sm:flex-row sm:items-center sm:justify-between">
                        <div className="min-w-0">
                          <p className="font-semibold text-[#544331] leading-tight">{p.referrerName ?? "—"}</p>
                          <p className="text-[11px] text-muted-foreground truncate">{p.referrerEmail}</p>
                          <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-[#5F4B3D]/75">
                            <span className="inline-flex items-center gap-1">
                              <Users size={11} className="text-[#836A5D]" />
                              Refirió a <strong className="text-[#544331]">{p.totalReferred}</strong> · <strong className="text-emerald-700">{p.totalConverted}</strong> compraron
                            </span>
                            {p.referredName && (
                              <span>Última: <strong className="text-[#544331]">{p.referredName}</strong></span>
                            )}
                            <span className="rounded-full bg-[#836A5D]/10 px-2 py-0.5 font-semibold text-[#725D51]">−{Number(p.discountPercent)}%</span>
                          </div>
                        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            {metric("Total referidos", summary?.total ?? 0, <Gift size={15} />, "#725D51")}
            {metric("Pendientes", summary?.pending ?? 0, <Clock size={15} />, "#8F7559")}
            {metric("Canjeados", summary?.rewarded ?? 0, <CheckCircle2 size={15} />, "#5F4B3D")}
            {metric("Conversión", `${summary?.conversion ?? 0}%`, <TrendingUp size={15} />, "#BEA98F")}
          </div>
                  ...
                  {summary.top.map((t, i) => (
                    <div key={t.id} className="flex items-center justify-between py-2.5">
                      <div className="flex items-center gap-3 min-w-0">
                        <span className="font-bebas text-lg text-[#725D51] w-5 text-center tabular-nums">{i + 1}</span>
                        <div className="min-w-0">
                          <p className="font-medium text-sm text-[#544331] truncate">{t.name}</p>
                          <p className="text-[11px] text-muted-foreground truncate">{t.email}</p>
                        </div>
                      </div>
                      <div className="text-right shrink-0 tabular-nums">
                        <p className="text-sm font-semibold text-[#544331]">{t.total} referida{t.total !== 1 ? "s" : ""}</p>
            ...
            <CardContent className="p-0">
              {loadingList ? (
                ...
                <div className="text-center py-12">
                  <Gift size={28} className="mx-auto text-[#BEA98F]/50 mb-2" />
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-[#836A5D]/15 text-[11px] uppercase tracking-wide text-muted-foreground">
                    ...
                    <tbody>
                      {filtered.map((r) => (
                        <tr key={r.id} className="border-b border-[#836A5D]/10 last:border-0 hover:bg-[#725D51]/[0.03]">
                          <td className="py-3 px-4">
                            <p className="font-medium text-[#544331] leading-tight">{r.referrerName ?? "—"}</p>
                            <p className="text-[11px] text-muted-foreground truncate max-w-[200px]">{r.referrerEmail}</p>
                          </td>
                          <td className="py-3 px-4">
                            <code className="text-[11px] font-mono text-[#725D51] bg-[#BEA98F]/15 px-1.5 py-0.5 rounded">{r.code ?? "—"}</code>
                          </td>
                          <td className="py-3 px-4">
                            <p className="font-medium text-[#544331] leading-tight">{r.referredName ?? "—"}</p>
