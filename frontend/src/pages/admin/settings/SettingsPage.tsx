...
const CancellationSettings = () => {
  ...
  return (
    <div className="space-y-5 max-w-md">
      <div className="flex items-center justify-between gap-3 rounded-xl border border-[#3B0E1A]/15 bg-white/40 px-4 py-3">
        <div>
          <Label className="text-sm font-medium text-[#260910]">Permitir cancelaciones</Label>
          <p className="text-xs text-[#260910]/55 mt-0.5">Si lo desactivas, nadie podrá cancelar desde la app.</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <Label className="text-sm font-medium text-[#260910]">Horas mínimas antes de la clase</Label>
          <Input
            type="number"
            min={0}
            ...
            disabled={!enabled}
          />
          <p className="text-xs text-[#260910]/55">Entre 0 y {CANCEL_MAX_HOURS} horas.</p>
        </div>
        <div className="space-y-1.5">
          <Label className="text-sm font-medium text-[#260910]">Cancelaciones gratis por membresía</Label>
          <Input
            type="number"
            min={0}
            max={31}
            ...
            disabled={!enabled}
          />
          <p className="text-xs text-[#260910]/55">Las siguientes cancelan, pero la clase se cuenta como tomada.</p>
        </div>
      </div>

      <div className="flex items-center justify-between gap-3 rounded-xl border border-[#3B0E1A]/15 bg-white/40 px-4 py-3">
        <div>
          <Label className="text-sm font-medium text-[#260910]">Devolver crédito al cancelar a tiempo</Label>
          <p className="text-xs text-[#260910]/55 mt-0.5">Si lo desactivas, la clase nunca regresa al paquete.</p>
        </div>
        <Switch checked={refund} onCheckedChange={setRefund} disabled={!enabled} />
      </div>

      <div className="space-y-1.5">
        <Label className="text-sm font-medium text-[#260910]">Mensaje cuando ya no se puede cancelar</Label>
        <Textarea
          rows={3}
        ...
        />
        <div className="flex items-center justify-between text-xs text-[#260910]/55">
      ...
      <div className="rounded-xl border border-[#3B0E1A]/25 bg-[#3B0E1A]/[0.06] px-4 py-3">
        <p className="text-[10px] font-bold tracking-[0.18em] uppercase text-[#3B0E1A]/70 mb-1.5">Vista previa</p>
        <p className="text-sm text-[#260910] leading-snug">{previewLine}</p>
      </div>

      {error && <p className="text-xs text-destructive">{error}</p>}

      <Button onClick={handleSave} disabled={updateMutation.isPending} className="bg-[#3B0E1A] hover:bg-[#320C16] text-[#EADCDD]">
...
const PaymentValidationSettings = () => {
  ...
  return (
    <div className="space-y-4 max-w-md mb-6 rounded-2xl border border-[#3B0E1A]/15 bg-white/40 p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <Label className="text-sm font-medium text-[#260910]">Validar transferencias manualmente</Label>
          <p className="text-xs text-[#260910]/55 mt-0.5 leading-snug">
      ...
      <div className="space-y-1 border-t border-[#3B0E1A]/10 pt-3">
        <Label className="text-sm font-medium text-[#260910]">Avisarme por WhatsApp</Label>
        <p className="text-xs text-[#260910]/55 mb-1.5 leading-snug">
      ...
      <Button onClick={() => updateMutation.mutate()} disabled={updateMutation.isPending} size="sm" className="bg-[#3B0E1A] hover:bg-[#320C16] text-[#EADCDD]">
        {updateMutation.isPending ? <Loader2 className="animate-spin mr-2" size={14} /> : null}
        Guardar
      </Button>
    </div>
...
const VenueMediaSettings = () => {
  ...
  return (
    <div className="rounded-xl border p-4 space-y-4 max-w-2xl">
      ...
      {mediaUrl ? (
        <div className="space-y-2">
          <div className="rounded-lg border border-border overflow-hidden bg-[#3B0E1A]/10">
            {mediaType === "video" ? (
