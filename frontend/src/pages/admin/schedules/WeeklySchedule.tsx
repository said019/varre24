...
const WeeklySchedule = () => {
  ...
  const scheduleCard = (s: Schedule) => (
    <div key={s.id} className="mb-2 p-2.5 bg-[#3B0E1A]/[0.05] rounded-xl border border-[#3B0E1A]/12 text-xs">
      <div className="font-semibold text-[#1A060B]/80 text-[11px] truncate">{s.classTypeName ?? s.classTypeId}</div>
      <div className="text-[#1A060B]/50 text-[10px] mt-0.5">{s.startTime}–{s.endTime}</div>
      <div className="text-[#1A060B]/35 text-[10px] truncate">{s.instructorName ?? s.instructorId}</div>
      <div className="flex items-center justify-between mt-1.5">
        <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full border ${s.isActive ? "text-[#4ade80] border-[#4ade80]/30 bg-[#4ade80]/5" : "text-[#1A060B]/25 border-[#3B0E1A]/15"}`}>
          {s.isActive ? "Activo" : "Inactivo"}
        </span>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" className="h-7 w-7 min-h-[44px] min-w-[44px] text-[#1A060B]/20 hover:text-[#1A060B]/60">
              <MoreHorizontal size={12} />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="bg-[#E9D9D9] border-[#3B0E1A]/15">
            <DropdownMenuItem className="text-[#1A060B]/70 hover:text-[#1A060B]" onClick={() => openEdit(s)}>Editar</DropdownMenuItem>
    ...
    <AuthGuard>
      <AdminLayout>
        <div className="admin-page max-w-5xl">
          <div className="mb-6 flex flex-col gap-4 rounded-2xl border border-[#3B0E1A]/15 bg-[#3B0E1A]/[0.04] p-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="admin-title font-bold text-[#1A060B]">Horarios semanales</h1>
          ...
          {isMobile ? (
            <div className="space-y-3">
                  ...
                  {DAYS.map((day, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="mt-0.5 text-[10px] text-[#1A060B]/55">clases</span>
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="rounded-xl border border-[#3B0E1A]/15 bg-[#3B0E1A]/[0.04] p-3">
                <div className="mb-3 flex items-center justify-between gap-2">
                  <div>
                    <p className="text-xs uppercase tracking-widest text-[#1A060B]/45">{DAYS[mobileDay].slice(0, 3)}</p>
                    <p className="text-sm font-semibold text-[#1A060B]">{DAYS[mobileDay]}</p>
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-7 gap-3">
              {DAYS.map((day, i) => (
                <div key={i} className="rounded-2xl border border-[#3B0E1A]/15 bg-[#3B0E1A]/[0.04] p-3">
                  <p className="text-[10px] font-bold text-center mb-3 text-[#3B0E1A]/60 uppercase tracking-widest">
                    {day.slice(0, 3)}
                  </p>
                  {grouped[i].length === 0 ? (
                    <p className="text-center text-[#1A060B]/15 text-xs py-3">—</p>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent className="max-w-md bg-[#E9D9D9] border-[#3B0E1A]/15 text-[#1A060B]">
            <DialogHeader>
              <DialogTitle className="text-[#1A060B]">{editing ? "Editar horario" : "Nuevo horario"}</DialogTitle>
            </DialogHeader>
            <form
            ...
            >
              <div className="space-y-1">
                <Label className="text-[#1A060B]/60 text-xs">Día</Label>
                <Select
                ...
                >
                  ...
                  <SelectContent className="bg-[#E9D9D9] border-[#3B0E1A]/15">
                    {DAYS.map((d, i) => (
                      <SelectItem key={i} value={String(i)} className="text-[#1A060B]">{d}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <Label className="text-[#1A060B]/60 text-xs">Activo</Label>
              </div>
              <DialogFooter>
                <Button
                  type="button"
