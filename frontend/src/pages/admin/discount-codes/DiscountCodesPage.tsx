      ...
      <AdminLayout>
        <div className="space-y-5">
          ...
          <div className="flex items-start justify-between gap-3 flex-wrap">
            ...
            <Button onClick={openCreate} className="bg-[#836A5D] hover:bg-[#6C5147] text-white">
          ...
          {isLoading ? (
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {codes.map((c) => {
                ...
                return (
                  <div key={c.id} className={`rounded-xl border p-4 transition-shadow hover:shadow-sm ${inactive ? "border-gray-300/50 bg-gray-50/40 opacity-70" : "border-[#836A5D]/15 bg-white"}`}>
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => copyCode(c.code)}
                            className="font-mono font-bold text-lg text-[#2d2d2d] hover:text-[#836A5D] transition-colors"
                            title="Copiar código"
                          >
                            {c.code}
                          </button>
                          <button
                            onClick={() => copyCode(c.code)}
                            className="text-[#836A5D]/40 hover:text-[#836A5D]"
                          >
                            <Copy size={12} />
                          </button>
                        </div>
                        <p className="text-sm font-semibold text-[#836A5D] mt-0.5 flex items-center gap-1">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-[11px] text-[#715B50]">
                      <span className="rounded-full bg-[#836A5D]/[0.06] px-2 py-0.5">
                      ...
                      {c.channel && c.channel !== "all" && (
                        <span className="rounded-full bg-[#836A5D]/[0.06] px-2 py-0.5">Canal: <strong>{c.channel}</strong></span>
                      )}
                    </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input
                  ...
                  disabled={!!editing}
                />
                {editing && <p className="text-[10px] text-[#836A5D]/60 mt-1">El código no se puede cambiar después de crearlo.</p>}
              </div>
              ...
              <div className="flex items-center justify-between rounded-lg border border-[#836A5D]/15 bg-[#836A5D]/[0.04] px-3 py-2">
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                ...
                {redemptionsFor && (
                  <span className="font-mono text-sm bg-[#836A5D]/10 text-[#836A5D] px-2 py-0.5 rounded">
                    {redemptionsFor.code}
                  </span>
                )}
              </DialogTitle>
            ...
            <div className="space-y-3 max-h-[60vh] overflow-y-auto">
              {redemptionsQuery.isLoading ? (
                <div className="flex items-center justify-center py-10 text-[#836A5D]/70 text-sm">
                  <Loader2 size={16} className="animate-spin mr-2" /> Cargando…
                </div>
              ) : redemptions.length === 0 ? (
                <p className="text-center py-10 text-sm text-[#715B50] italic">
                  Nadie ha usado este cupón todavía.
                </p>
              ) : (
                <>
                  <p className="text-[11px] uppercase tracking-[0.14em] text-[#836A5D]/60 font-semibold px-1">
                    {redemptions.length} {redemptions.length === 1 ? "uso" : "usos"} · más recientes primero
                  </p>
                  <ul className="divide-y divide-[#836A5D]/10 rounded-xl border border-[#836A5D]/15 bg-white">
                    {redemptions.map((r) => {
                      ...
                      return (
                        <li key={r.orderId} className="p-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                          <div className="min-w-0">
                            <p className="text-sm font-semibold text-[#2d2d2d] truncate">{r.userName}</p>
                            <p className="text-[11px] text-[#836A5D]/70 truncate">
                              {r.userEmail} · {r.planName ?? "—"} · <span className="font-mono">{r.orderNumber}</span>
                            </p>
                            <p className="text-[11px] text-[#836A5D]/55">
                          ...
                          <div className="flex flex-col items-end gap-1 shrink-0 tabular-nums">
                            ...
                            <p className="text-sm font-bold text-[#2d2d2d]">${r.totalAmount.toLocaleString("es-MX", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
