  ...
  if (isMobile) {
    return (
      <div className={cn("relative w-full", className)}>
        <CalendarDays size={14} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#3B0E1A]" />
        <input
          type="date"
          min={min}
          value={value ?? ""}
          ...
          className={cn(
            "w-full rounded-xl border border-[#3B0E1A]/20 bg-white py-2.5 pl-9 pr-3 text-sm text-[#1A060B]",
            "focus:border-[#3B0E1A]/50 focus:outline-none",
  ...
  return (
    <div ref={ref} className={cn("relative inline-block w-full", className)}>
      ...
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        className={cn(
          "flex w-full items-center gap-2.5 rounded-xl border border-[#3B0E1A]/20 bg-white px-3 py-2.5 text-sm transition-all",
          "hover:border-[#3B0E1A]/40 focus:outline-none",
          open ? "border-[#3B0E1A]/50 ring-1 ring-[#3B0E1A]/20" : "",
          disabled && "opacity-50 pointer-events-none",
        )}
      >
        <CalendarDays size={14} className="shrink-0 text-[#3B0E1A]" />
        <span className={cn("flex-1 text-left", selected ? "text-[#1A060B] font-medium" : "text-[#3B0E1A]/50")}>
          {selected
        ...
        <ChevronRight
          size={13}
          className={cn("text-[#3B0E1A]/40 transition-transform", open && "rotate-90")}
        />
      </button>
      ...
      {open && typeof document !== "undefined" && createPortal(
        <div
          ...
          className="rounded-2xl border border-[#3B0E1A]/15 bg-white shadow-xl shadow-black/10 p-4"
        >
          {/* Month nav */}
          <div className="flex items-center justify-between mb-3">
            <button
              ...
              className="w-7 h-7 rounded-lg flex items-center justify-center text-[#3B0E1A]/50 hover:text-[#3B0E1A] hover:bg-[#3B0E1A]/10 transition-all"
            >
              <ChevronLeft size={13} />
            </button>
            <span className="text-sm font-semibold text-[#1A060B] capitalize">
          ...
          <div className="grid grid-cols-7 mb-1">
            {DAYS_SHORT.map((d) => (
              <div key={d} className="text-center text-[10px] font-semibold text-[#3B0E1A]/60 py-1">
                {d}
              </div>
            ))}
          </div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSelected
                      ? "bg-[#3B0E1A] text-white shadow-sm"
                      : isCurrentDay && !isSelected
                        ? "border border-[#C9A5A8] text-[#1A060B] bg-[#C9A5A8]/10"
                        : isThisMonth
                          ? "text-[#1A060B] hover:bg-[#3B0E1A]/10"
                          : "text-[#3B0E1A]/30",
